 ...
 */
    ...
    /** Default sample size to work with */
     static  int DEFAULT_SAMPLE_SIZE = 256;

    /** The log of 1/2 constant. Used Everywhere */
     static  float LOG_HALF = (float)log(0.5);
    ...
    void PerlinNoiseGenerator(unsigned int seed)
    {
        ...
        {
            k = p_imp[i];
		    j = int(rand() & DEFAULT_SAMPLE_SIZE);
            p_imp[i] = p_imp[j];
            p_imp[j] = k;
        }
     ...
     * </pre>
     */
     double imporvedTurbulence(double x,
                                     double y,
                                     double z,
                                     float loF,
                                     float hiF)
    {
        ...
        for(f = loF; f < hiF; f *= 2)
        {
			double noise = improvedNoise(p_x, p_y, p_z);
     ...
     * @return The value at the given coordinates
     */
     float tileableTurbulence2(float x,
                                     float y,
                                     float w,
                                     float h,
                                     float freq)
     ...
     * @return The value at the given coordinates
     */
     float tileableTurbulence3(float x,
                                     float y,
                                     float z,
                                     float w,
                                     float h,
    ...
    {
        return (float)pow(a, log(b) / LOG_HALF);
    }
     ...
     float gain(float a, float b)
    {
        ...
        double p = log(1.0f - b) / LOG_HALF;

        if(a < 0.5f)
			return (float)(pow((double)(2 * a), p) / 2);
        else
			return 1 - (float)(pow((double)(2 * (1.0 - a)), p) / 2);
    }
    ...
    {
        float s = (float)(1 / sqrt(v[0] * v[0] + v[1] * v[1]));
        v[0] *= s;
        v[1] *= s;
    }
    ...
    {
        float s = (float)(1 / sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]));
        v[0] *= s;
        v[1] *= s;
        v[2] *= s;
    }
...
//            g1[i] = float( long((NOise(RAND)*RAND)) % (B + B) - B) / B;
            g1[i] = float(rand() % (B + B) - B) / B;
			
            for(j = 0; j < 2; j++)
  //              g2[i][j] = float( long((NOise(RAND)*RAND)) % (B + B) - B) / B;
                g2[i][j] = float(rand() % (B + B) - B) / B;
            normalize2(g2[i]);
			
            for(j = 0; j < 3; j++)
//                g3[i][j] = float( long((NOise(RAND)*RAND)) % (B + B) - B) / B;
