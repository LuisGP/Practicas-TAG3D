...
static float RELIEVE = 1.0; // Controla el tamaño vertical del terreno
static float ESCALA = 3.0; // Controla el tamaño horizontal del terreno
...
void setAlturaPj(){
	...
	if(acamX > 2 && acamX < ALTO-2 && acamZ > 2 && acamZ < ANCHO-2){
		...
		camY = max(NIVEL_LAGO+alturaPjNado, aux);
	}
...
void init(){
	...
	setSunLight(horas, minutos, 0, posicion_luz, luzdifusa, ESCALA);
	...
	paleta = loadPalInt("paletas/landcolor2.ppm", 256);
	...
	paleta_grass = loadPalInt("paletas/landcolor.ppm", 256);
...
void applyBumpMapping(){
	...
	for(i = 0; i < ALTO; i++){
		for(j = 0; j < ANCHO; j++){
			...
			//if(NIVEL_LAGO >= alturas[i][j]) // El lago (lo pintamos con otro plano ^^)
...
void actualizaSol(bool forzar){
	...
	//setSunLight(horas, minutos, 0, posicion_luz, luzdifusa, ESCALA); MUDANZA
	
	
	/*
	 // Este método es más suave que el otro... ;)
	 
	 clock_t act = clock()*CLK_TCK;
	 float diff = act - tiempo;
	 
	 // El 1 seg / escala_de_tiempo ;)
	 if(diff > (1000000/10)){ // 0.1 segundos... usease, como el x10
	 if(!forzar)
	 minutos++;
	 if(minutos == 60){
	 minutos = 0;
	 ...
	 sprintf(wtitle, "%s - %d:%.2d  FPS: %d", myProgramName, horas%24, minutos, fps);
	 glutSetWindowTitle(wtitle);
	 setSunLight(horas, minutos, 0, posicion_luz, luzdifusa);
	 tiempo = act;
	 }
	 */
	/*
	 if(time(NULL) > tiempo+1 || forzar){
	 //cerr << minutos << "'"<< endl;
	 if(!forzar)
	 minutos+=1;
...
draw()
{
	...
	//	actualizaSol(false); //////////////// DE MOMENTO LO QUITO DE AQUI
	...
	setSunLight(horas, minutos, 0, posicion_luz, luzdifusa, ESCALA); ///////// ESTA AQUI PARA QUE ILUMINE DONDE DEBE
	...
	// Evitamos errores con Z-Buffer
	glNormal3f(0,1,0);            // <--- OJO, TAMBIEN ES TEMPORAL MAS O MENOS
	alpha = anguloLago((M_ALTO-1)*ESCALA, NIVEL_LAGO*RELIEVE+0.5, -M_ANCHO*ESCALA);
	glColor4f(0.5,0.5,0.5,alpha);
	//setIntColorMaterial (paleta_agua[0], alpha);
	glVertex3f((M_ALTO-1)*ESCALA, NIVEL_LAGO*RELIEVE+0.5, -M_ANCHO*ESCALA);
	
	alpha = anguloLago(-M_ALTO*ESCALA, NIVEL_LAGO*RELIEVE+0.5, -M_ANCHO*ESCALA);
	glColor4f(0.5,0.5,0.5,alpha);
	//setIntColorMaterial (paleta_agua[0], alpha);
	glVertex3f(-M_ALTO*ESCALA, NIVEL_LAGO*RELIEVE+0.5, -M_ANCHO*ESCALA);
	
	alpha = anguloLago(-M_ALTO*ESCALA, NIVEL_LAGO*RELIEVE+0.5, (M_ANCHO-1)*ESCALA);
	glColor4f(0.5,0.5,0.5,alpha);
	//setIntColorMaterial (paleta_agua[0], alpha);
	glVertex3f(-M_ALTO*ESCALA, NIVEL_LAGO*RELIEVE+0.5, (M_ANCHO-1)*ESCALA);
	
	alpha = anguloLago((M_ALTO-1)*ESCALA, NIVEL_LAGO*RELIEVE+0.5, (M_ANCHO-1)*ESCALA);
	setIntColorMaterial (paleta_agua[0], alpha);
	glVertex3f((M_ALTO-1)*ESCALA, NIVEL_LAGO*RELIEVE+0.5, (M_ANCHO-1)*ESCALA);
...
static void keyboard(unsigned char key, int x, int y)
{
	...
	switch (key) {
		...
		case 'W':
			...
			velocidad *= 2;
			if (velocidad > 32768 ) velocidad = 32768;
