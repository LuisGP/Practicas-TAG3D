...
void setSunLight (int h, int m, float clouds){
   ...
   GLfloat position[] = {128-sin(ang)*1024, -cos(ang)*1024, 128, 1.0};
   GLfloat clearColor[4]; // Color del cielo

   clearColor[3] = 1;
   // Color en funcion del angulo
   GLfloat r = 1;
   float posf = (position[1])/1024.0;
   if (h == 17 && m > 49){
		r = (10-(m-49))/10.0;
	}
   if (h == 6 && m < 10){
  ...
  cout << h<<":"<<m<<" dif: "<< r << " " << g << " " << b;
   ...
   if (position[1] < -0.5) b=0.15;
	...
	if (h > 6 && h < 17 || h==6 && m>30 || h == 17 && m<30){
		r = 0.1;
		g = .1;
		b = .15;
	}
   ...
   glLightfv(GL_LIGHT0, GL_POSITION, position);
	...
	clearColor[0] = 0.3;
	if ( h==6 && m < 30){
		clearColor[0] = 1-((mins-360)/30.0)*.7;
		if (clearColor[0] > 1) clearColor[0] = 1;
		if (m < 5)
			clearColor[0] = ((mins-360)/5.0)*.7;
	}
	if ( h==17 && m>30 ){ /////// FALTA PROBARLO
		int submins = 1440-mins;
		clearColor[0] = 1-((submins-360)/30.0)*.7;
		if (clearColor[0] > 1) clearColor[0] = 1;
		if (m > 55)
			clearColor[0] = ((submins-360)/5.0)*.7;
	}

	// VERDE
	clearColor[1] = 0.7;
	if ( h==6 && m < 18){
		clearColor[1] = ((mins-363)/15.0)*.7;
		if (clearColor[1] > 1) clearColor[1] = 1;
	}
	if ( h==17 && m > 42){ /////// FALTA PROBARLO
		int submins = 1440-mins;
		clearColor[1] = ((submins-363)/15.0)*.7;
		if (clearColor[1] > 1) clearColor[1] = 1;
	}
	...
	clearColor[2] = 1;
	if ( h==6 && m < 30){
		clearColor[2] = ((mins-365)/25.0);
		cout << "MS:" << mins-365 << endl;
		if (clearColor[2] < 0) clearColor[2] = 0;
		if (clearColor[2] > 1) clearColor[2] = 1;
	}
	if ( h==17 && m > 30 ){ /////// FALTA PROBARLO
		int submins = 1440-mins;
		clearColor[2] = ((submins-365)/25.0);
		cout << "MS:" << submins-365 << endl;
		if (clearColor[2] < 0) clearColor[2] = 0;
		if (clearColor[2] > 1) clearColor[2] = 1;
	}
   ...
   cout << "     sky: "<< clearColor[0] << " " << clearColor[1] << " " << clearColor[2] << endl;
